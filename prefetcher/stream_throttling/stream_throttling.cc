...
void update_aggressiveness(uint64_t pf_useful, uint64_t pf_issued) {
  if (pf_issued != 0) {
    double accuracy = (double) pf_useful / pf_issued;

    if (accuracy > 0.8) {
      CURRENT_PREFETCH_LEVEL = 4;
    } else if (accuracy > 0.6) {
      CURRENT_PREFETCH_LEVEL = 3;
    } else if (accuracy > 0.4) {
      CURRENT_PREFETCH_LEVEL = 2;
    } else if (accuracy > 0.2) {
      CURRENT_PREFETCH_LEVEL = 1;
    } else {
      CURRENT_PREFETCH_LEVEL = 0;
    }

    PREFETCH_DEGREE = PREFETCH_LEVELS[CURRENT_PREFETCH_LEVEL][0];
    PREFETCH_DISTANCE = PREFETCH_LEVELS[CURRENT_PREFETCH_LEVEL][1];
  }
}
