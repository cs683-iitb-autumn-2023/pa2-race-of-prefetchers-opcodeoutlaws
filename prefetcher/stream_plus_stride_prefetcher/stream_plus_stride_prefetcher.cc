...
void CACHE::prefetcher_cycle_operate()
{
  ...
  if (auto [old_pf_address, direction, degree, stride] = lookahead[this]; degree > 0) {
    uint64_t pf_address;
    ...
    if (virtual_prefetch || (pf_address >> LOG2_PAGE_SIZE) == (old_pf_address >> LOG2_PAGE_SIZE)) {
      ...
      if (success) {
        lookahead[this] = {pf_address, direction, degree - 1, stride};
        // printf("prefetched %u old pf adddress = %u and direction = %d and stride = %d\n", pf_address >> LOG2_BLOCK_SIZE, old_pf_address >> LOG2_BLOCK_SIZE, direction, stride);
...
uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in)
{
  ...
  if (!cache_hit) {
      ...
      if (found->state == ALLOCATED) {
        ...
        if ((cl_addr >= found->A && cl_addr <= found->P) || (cl_addr <= found->A && cl_addr >= found->P)) {
          ...
          if (stride != 0 && stride == found->last_stride) {
            ...
            uint64_t start_addr = found->P;
            int diff = found->P - cl_addr;
            if (diff % stride != 0) {
              start_addr = found->P + stride - (diff % stride);
            }

            lookahead[this] = {start_addr << LOG2_BLOCK_SIZE, found->direction, PREFETCH_DEGREE, stride};
          }
    ...
    *found = {ip, cl_addr, new_A, new_P, direction, current_cycle, state, stride};
  }
  return metadata_in;
}
